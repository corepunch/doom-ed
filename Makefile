...
# Libraries (libgoldie built via ui/Makefile)
ifeq ($(UNAME_S),Darwin)
  LIBGOLDIE = $(BUILD_DIR)/libgoldie.dylib
else
  LIBGOLDIE = $(BUILD_DIR)/libgoldie.so
endif
...
# libgoldie library (built via ui/Makefile)
libgoldie: $(LIBGOLDIE)

$(LIBGOLDIE):
	@echo "Building libgoldie via ui/Makefile..."
	@$(MAKE) -C $(UI_DIR) all
